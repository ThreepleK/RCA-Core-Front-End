...
import { UI_Title } from '@/compos/ui';
...
function Layout({ children }: {
    ...
    const className = useStore(stateStore, s => s.className);       // 컨텐츠 사이드 영역 넓이
        ...
        <section className={style.section} style={{
            ...
            >
                ...
                <div className={style['ta-left']}>
                    ...
                    {/* 타이틀 */}
                    <UI_Title order={5} className={style.title}>{titleLeft}</UI_Title>
                </div>
            ...
            {/* 본문 영역 */}
            <div className={`${style['cont-area']} ${className}`}>{children}</div>
        </section>
    </>;
}
