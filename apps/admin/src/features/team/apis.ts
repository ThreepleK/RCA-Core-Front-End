 ...
 * 앱 메뉴 저장하기
 */
export function api_setMenuData(data: any[]){
    return request({
        type: 'post',
        url: '/admin/api/menu/application',
        datas: data
 ...
 */
export function api_team_list(opts: {
    pagination: MRT_PaginationState,
    search: string,
    filter: MRT_ColumnFilterFnsState[],
    sort: MRT_SortingState[],
     ...
     return new Promise((res) => {
         setTimeout(() => {
             res({
                 data: JSON.parse(JSON.stringify(_TMP_TEAM_DATA)),
                 meta: { total: 200 },
             });
         }, 1000);
     });
 ...
 * @param id 사용자 고유 아이디
 */
export function api_chkDupleTeamName(name: string, id?: string){
    // return request({
    //     type: 'post',
    //     url: '/admin/api/*',
    //     datas: {email}
    ...
    return new Promise((resolve) => {
        ...
        // 중복 이메일 체크
        const isDuple = ((chkName: string) => {
            for( const item of _TMP_DATA ){
                // 다른 사용자의 이메일 주소와 같은지 확인
                if( item.name === chkName && (item?.id && item.id !== id) ){
                    return true;
                }
            }
            return false;
