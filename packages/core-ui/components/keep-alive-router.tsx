...
function getActiveKey(
    target: string,     // 대상  (/, /rca, /dashboard, /admin 등)
 ...
 * ActiveKey 파서
 */
function activeKeyParser(key: string){
    const [target, url] = key.split('<,>');
    return {target, url};
}
 ...
 */
function MemoScrollTopWrapper({ target, children, aliveRef }: {
    target: string;
    children?: ReactNode;
    aliveRef: RefObject<KeepAliveRef | undefined>;
}) {
    ...
    useEffect(() => {
        ...
        // console.log(target, aliveRef.current?.getCacheNodes());
...
function CustomSuspense(props: { children: ReactNode }) {
    ...
    return <Suspense fallback={<div className="flex justify-center items-center text-[12px] w-full h-full">Loading...</div>}>{children}</Suspense>;
}